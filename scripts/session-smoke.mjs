...
async function runFlow(mode, port) {
  ...
  try {
    ...
    const textoPagina = await page.locator("main").innerText();
    const numeradas = [
      "1. Mi personaje",
      "2. Build objetivo",
      "3. Mercado actual",
      "4. Próximas mejoras",
      "5. Habla con tu mentor",
    ].filter((titulo) => textoPagina.includes(titulo));
    check(
      `[${mode}] no reaparece la navegación antigua numerada`,
      numeradas.length === 0,
      numeradas.join(" · "),
    );
    check(
    ...
    await page.waitForFunction(
      () => {
        const boton = Array.from(document.querySelectorAll("button")).find((b) =>
          b.textContent?.includes("Generar recomendaciones"),
        );
